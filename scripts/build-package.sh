...
rustup run nightly-2019-09-01 wasm-pack build --release --scope plugnet --target nodejs
mv pkg build
...
sed -i -e 's/const TextEncoder = require('\''util'\'')\.TextEncoder;/const { stringToU8a } = require('\''@plugnet\/util'\'');/g' $SRC_WASM
...
# this creates issues in both the browser and RN (@plugnet/util has a polyfill)
sed -i -e 's/const TextDecoder = require('\''util'\'')\.TextDecoder;/const { u8aToString } = require('\''@plugnet\/util'\'');/g' $SRC_WASM
